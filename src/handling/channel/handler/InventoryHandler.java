...
public class InventoryHandler {
    ...
    public static final void ItemGather(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        for (IItem itemStats : itemMap) {
            if (itemStats.getItemId() != 5_110_000) {
    ...
    public static final void UseItem(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || !chr.isAlive() || chr.getMapId() == 749_040_100 || chr.getMap() == null/*
    ...
    public static final void UseReturnScroll(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        if (!chr.isAlive() || chr.getMapId() == 749_040_100) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
        c.getPlayer().updateTick(slea.readInt());
    ...
    public static final void UseMagnify(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        if (eqq.getState() == 1 && (magnify.getItemId() == 2_460_003 || (magnify.getItemId() == 2_460_002 && reqLevel <= 12) || (magnify.getItemId() == 2_460_001 && reqLevel <= 7) || (magnify.getItemId() == 2_460_000 && reqLevel <= 3))) {
    ...
    public static final boolean UseUpgradeScroll(final byte slot, final byte dst, final byte ws, final MapleClient c, final MapleCharacter chr, final int vegas) {
        ...
        if (whiteScroll) {
            wscroll = chr.getInventory(MapleInventoryType.USE).findById(2_340_000);
            if (wscroll == null) {
                whiteScroll = false;
            }
        }
        if (scroll.getItemId() == 2_049_115 && toScroll.getItemId() != 1_003_068) {
            //ravana
            return false;
        }
        if (GameConstants.isTablet(scroll.getItemId())) {
            switch (scroll.getItemId() % 1_000 / 100) {
                case 0: //1h
        ...
        if (legendarySpirit && vegas == 0) {
            if (chr.getSkillLevel(SkillFactory.getSkill(1_003)) <= 0 && chr.getSkillLevel(SkillFactory.getSkill(10_001_003)) <= 0 && chr.getSkillLevel(SkillFactory.getSkill(20_001_003)) <= 0 && chr.getSkillLevel(SkillFactory.getSkill(20_011_003)) <= 0 && chr.getSkillLevel(SkillFactory.getSkill(30_001_003)) <= 0) {
                AutobanManager.getInstance().addPoints(c, 50, 120_000, "Using the Skill 'Legendary Spirit' without having it.");
                return false;
            }
        }
    ...
    public static final void UseCatchItem(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse != null && toUse.getQuantity() > 0 && toUse.getItemId() == itemid && mob != null) {
            switch (itemid) {
                case 2_270_004: { //Purification Marble
                    ...
                    if (mob.getHp() <= mob.getMobMaxHp() / 2) {
                        ...
                        MapleInventoryManipulator.addById(c, 4_001_169, (short) 1, (byte) 0);
                    } else {
                ...
                case 2_270_002: { // Characteristic Stone
                    final MapleMap map = chr.getMap();
                ...
                case 2_270_000: { // Pheromone Perfume
                    if (mob.getId() != 9_300_101) {
                        break;
                    }
                    ...
                    map.broadcastMessage(MaplePacketCreator.catchMonster(mob.getId(), itemid, (byte) 1));
                    map.killMonster(mob, chr, true, false, (byte) 0);
                    MapleInventoryManipulator.addById(c, 1_902_000, (short) 1, null, (byte) 0);
                    MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, itemid, 1, false, false);
                    break;
                }
                case 2_270_003: { // Cliff's Magic Cane
                    if (mob.getId() != 9_500_320) {
                        break;
                    }
                    final MapleMap map = c.getPlayer().getMap();
    ...
    public static final void UseScriptedNPCItem(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId) {
            ...
            switch (toUse.getItemId()) {
                ...
                case 2_430_007: // Blank Compass
                {
                    final MapleInventory inventory = chr.getInventory(MapleInventoryType.SETUP);
                    MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);

                    if (inventory.countById(3_994_102) >= 20 // Compass Letter "North"
                            && inventory.countById(3_994_103) >= 20 // Compass Letter "South"
                            && inventory.countById(3_994_104) >= 20 // Compass Letter "East"
                            && inventory.countById(3_994_105) >= 20) { // Compass Letter "West"
                        MapleInventoryManipulator.addById(c, 2_430_008, (short) 1, (byte) 0); // Gold Compass
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.SETUP, 3_994_102, 20, false, false);
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.SETUP, 3_994_103, 20, false, false);
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.SETUP, 3_994_104, 20, false, false);
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.SETUP, 3_994_105, 20, false, false);
                    } else {
                        MapleInventoryManipulator.addById(c, 2_430_007, (short) 1, (byte) 0); // Blank Compass
                    }
                    NPCScriptManager.getInstance().start(c, 2_084_001);
                    break;
                }
                case 2_430_008: // Gold Compass
                {
                    chr.saveLocation(SavedLocationType.RICHIE);
                    MapleMap map;
                    boolean warped = false;

                    for (int i = 390_001_000; i <= 390_001_004; i++) {
                    ...
                    if (warped) { // Removal of gold compass
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, 2_430_008, 1, false, false);
                    } else { // Or mabe some other message.
                        c.getPlayer().dropMessage(5, "所有地图都在使用，请稍后再试.");
                    }
                    break;
                }
                case 2_430_112: //miracle cube
                    if (c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                        if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2_430_112) >= 25) {
                            if (MapleInventoryManipulator.checkSpace(c, 2_049_400, 1, "") && MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, 2_430_112, 25, true, false)) {
                                MapleInventoryManipulator.addById(c, 2_049_400, (short) 1, (byte) 0);
                            } else {
                                c.getPlayer().dropMessage(5, "请清理空间.");
                            }
                        } else if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2_430_112) >= 10) {
                            if (MapleInventoryManipulator.checkSpace(c, 2_049_400, 1, "") && MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, 2_430_112, 10, true, false)) {
                                MapleInventoryManipulator.addById(c, 2_049_401, (short) 1, (byte) 0);
                            } else {
                                c.getPlayer().dropMessage(5, "请清理空间.");
                            }
                        } else {
                    ...
                    break;
                case 2_430_036: //croco 1 day
                    mountid = 1_027;
                    expiration_days = 1;
                    break;
                case 2_430_037: //black scooter 1 day
                    mountid = 1_028;
                    expiration_days = 1;
                    break;
                case 2_430_038: //pink scooter 1 day
                    mountid = 1_029;
                    expiration_days = 1;
                    break;
                case 2_430_039: //clouds 1 day
                    mountid = 1_030;
                    expiration_days = 1;
                    break;
                case 2_430_040: //balrog 1 day
                    mountid = 1_031;
                    expiration_days = 1;
                    break;
                case 2_430_053: //croco 30 day
                    mountid = 1_027;
                    expiration_days = 1;
                    break;
                case 2_430_054: //black scooter 30 day
                    mountid = 1_028;
                    expiration_days = 30;
                    break;
                case 2_430_055: //pink scooter 30 day
                    mountid = 1_029;
                    expiration_days = 30;
                    break;
                case 2_430_056: //mist rog 30 day
                    mountid = 1_035;
                    expiration_days = 30;
                    break;
                //race kart 30 day? unknown 2430057
                case 2_430_072: //ZD tiger 7 day
                    mountid = 1_034;
                    expiration_days = 7;
                    break;
                case 2_430_073: //lion 15 day
                    mountid = 1_036;
                    expiration_days = 15;
                    break;
                case 2_430_074: //unicorn 15 day
                    mountid = 1_037;
                    expiration_days = 15;
                    break;
                case 2_430_075: //low rider 15 day
                    mountid = 1_038;
                    expiration_days = 15;
                    break;
                case 2_430_076: //red truck 15 day
                    mountid = 1_039;
                    expiration_days = 15;
                    break;
                case 2_430_077: //gargoyle 15 day
                    mountid = 1_040;
                    expiration_days = 15;
                    break;
                case 2_430_080: //shinjo 20 day
                    mountid = 1_042;
                    expiration_days = 20;
                    break;
                case 2_430_082: //orange mush 7 day
                    mountid = 1_044;
                    expiration_days = 7;
                    break;
                case 2_430_091: //nightmare 10 day
                    mountid = 1_049;
                    expiration_days = 10;
                    break;
                case 2_430_092: //yeti 10 day
                    mountid = 1_050;
                    expiration_days = 10;
                    break;
                case 2_430_093: //ostrich 10 day
                    mountid = 1_051;
                    expiration_days = 10;
                    break;
                case 2_430_101: //pink bear 10 day
                    mountid = 1_052;
                    expiration_days = 10;
                    break;
                case 2_430_102: //transformation robo 10 day
                    mountid = 1_053;
                    expiration_days = 10;
                    break;
                case 2_430_103: //chicken 30 day
                    mountid = 1_054;
                    expiration_days = 30;
                    break;
                case 2_430_117: //lion 1 year
                    mountid = 1_036;
                    expiration_days = 365;
                    break;
                case 2_430_118: //red truck 1 year
                    mountid = 1_039;
                    expiration_days = 365;
                    break;
                case 2_430_119: //gargoyle 1 year
                    mountid = 1_040;
                    expiration_days = 365;
                    break;
                case 2_430_120: //unicorn 1 year
                    mountid = 1_037;
                    expiration_days = 365;
                    break;
                case 2_430_136: //owl 30 day
                    mountid = 1_069;
                    expiration_days = 30;
                    break;
                case 2_430_137: //owl 1 year
                    mountid = 1_069;
                    expiration_days = 365;
                    break;
                case 2_430_201: //giant bunny 60 day
                    mountid = 1_096;
                    expiration_days = 60;
                    break;
                case 2_430_228: //tiny bunny 60 day
                    mountid = 1_101;
                    expiration_days = 60;
                    break;
                case 2_430_229: //bunny rickshaw 60 day
                    mountid = 1_102;
                    expiration_days = 60;
                    break;
            }
        ...
        if (mountid > 0) {
            mountid += (GameConstants.isAran(c.getPlayer().getJob()) ? 20_000_000 : (GameConstants.isEvan(c.getPlayer().getJob()) ? 20_010_000 : (GameConstants.isKOC(c.getPlayer().getJob()) ? 10_000_000 : (GameConstants.isResist(c.getPlayer().getJob()) ? 30_000_000 : 0))));
            if (c.getPlayer().getSkillLevel(mountid) > 0) {
                c.getPlayer().dropMessage(5, "你已经拥有了这个技能.");
            } else if (expiration_days > 0) {
                MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
                c.getPlayer().changeSkillLevel(SkillFactory.getSkill(mountid), (byte) 1, (byte) 1, System.currentTimeMillis() + (long) (expiration_days * 24 * 60 * 60 * 1_000));
                c.getPlayer().dropMessage(5, "已经达到的技能.");
            }
        }
        if ((itemId >= 2_022_570 && itemId <= 2_022_573) && (itemId >= 2_022_575 && itemId <= 2_022_578) && itemId >= 2_022_580 && itemId <= 2_022_583) {
    ...
    public static final void UseTreasureChest(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getInventory(MapleInventoryType.CASH).countById(keyIDforRemoval) > 0) {
            ...
            if (item == null) {
                chr.dropMessage(5, "请确认是否有金钥匙或者你身上的空间满了.");
                c.getSession().write(MaplePacketCreator.enableActions());
                return;
            }
            ...
            if (GameConstants.gachaponRareItem(item.getItemId()) > 0) {
                World.Broadcast.broadcastMessage(MaplePacketCreator.getGachaponMega("[" + box + "] " + c.getPlayer().getName(), " : 从金宝箱中获得", item, (byte) 2, c.getPlayer().getClient().getChannel()).getBytes());
            }
        } else {
            chr.dropMessage(5, "请确认是否有银钥匙或者你身上的空间满了.");
            c.getSession().write(MaplePacketCreator.enableActions());
        }
    }
...
//        c.getPlayer().updateTick(slea.readInt());
        ...
        switch (itemId) {
            case 5_530_000:
                NPCScriptManager.getInstance().start(c, 9_900_004, 10_086);
                c.getSession().write(MaplePacketCreator.enableActions());
                break;
            case 5_042_000:
                c.getPlayer().changeMap(701_000_200);
                used = true;
                break;
            case 5_042_001:
                c.getPlayer().changeMap(741_000_000);
                used = true;
                break;
            case 5_043_001: // NPC Teleport Rock
            case 5_043_000: { // NPC Teleport Rock
            ...
            case 2_320_000: // The Teleport Rock
            case 5_041_000: // VIP Teleport Rock
            case 5_040_000: // The Teleport Rock
            case 5_040_001: { // Teleport Coke
                if (slea.readByte() == 0) { // Rocktype
                    final MapleMap target = c.getChannelServer().getMapFactory().getMap(slea.readInt());
                    if (target != null) {
                        if ((itemId == 5_041_000 && c.getPlayer().isRockMap(target.getId())) || (itemId != 5_041_000 && c.getPlayer().isRegRockMap(target.getId()))) {
                    ...
                    if (victim != null && !victim.isGM() && c.getPlayer().getEventInstance() == null && victim.getEventInstance() == null) {
                        if (!FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit()) && !FieldLimitType.VipRock.check(c.getChannelServer().getMapFactory().getMap(victim.getMapId()).getFieldLimit())) {
                            if (itemId == 5_041_000 || (victim.getMapId() / 100_000_000) == (c.getPlayer().getMapId() / 100_000_000)) { // Viprock or same continent
...
//                if (apfrom == 0x2000 && apto != 0x8000) { // 如果从hp洗到mp 中断
//                    c.sendPacket(MaplePacketCreator.enableActions());
//                    return;
//                } else if (apfrom == 0x8000 && apto != 0x2000) {// 如果从mp洗到hp 中断
//                    c.sendPacket(MaplePacketCreator.enableActions());
//                    return;
//                }
                switch (apto) { // AP to
                    ...
                    case 0x800: // luk
                        ...
                        break;
                    case 0x2000: // hp
                        if (playerst.getMaxHp() >= 30_000) {
                            used = false;
                        }
                        break;
                    case 0x8000: // mp
                        if (playerst.getMaxMp() >= 30_000) {
                            used = false;
                        }
                        break;
                }
                    ...
                    switch (apfrom) { // AP from
                        ...
                        case 8_192: // HP
                            ...
                            if (job == 0) { // Beginner
                                ...
                                maxhp -= 10;
                            } else if ((job >= 300 && job <= 322) || (job >= 400 && job <= 434) || (job >= 1_300 && job <= 1_312) || (job >= 1_400 && job <= 1_412) || (job >= 3_300 && job <= 3_312) || (job >= 3_500 && job <= 3_512)) { // Bowman, Thief
                                maxhp -= 15;
                                ...
                                maxhp -= 12;
                            } else if ((job >= 2_000 && job <= 2_112) || (job >= 3_200 && job <= 3_212)) { // Aran
                                maxhp -= 40;
                            } else { // GameMaster
                                maxhp -= 20;
                            }
                        ...
                        case 32_768: // MP
                            ...
                            } else if ((job >= 500 && job <= 522) || (job >= 300 && job <= 322) || (job >= 400 && job <= 434) || (job >= 1_300 && job <= 1_312) || (job >= 1_400 && job <= 1_412) || (job >= 1_500 && job <= 1_512) || (job >= 3_300 && job <= 3_312) || (job >= 3_500 && job <= 3_512)) { // Pirate, Bowman. Thief
                                maxmp -= 10;
                            } else if (job >= 1_100 && job <= 1_112) { // Soul Master
                                maxmp -= 6;
                            } else if (job >= 1_200 && job <= 1_212) { // Flame Wizard
            ...
            case 5_050_001: // SP Reset (1st job)
            case 5_050_002: // SP Reset (2nd job)
            case 5_050_003: // SP Reset (3rd job)
            case 5_050_004: { // SP Reset (4th job)
            ...
            case 5_062_000: { //miracle cube
                ...
                if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    if (eq.getState() >= 5) {
                        ...
                        MapleInventoryManipulator.addById(c, 2_430_112, (short) 1, (byte) 0);
                        used = true;
                    } else {
            ...
            case 5_520_000: { // Karma
                ...
                if (item != null && !ItemFlag.KARMA_EQ.check(item.getFlag()) && !ItemFlag.KARMA_USE.check(item.getFlag())) {
                    if (itemId == 5_520_000 && MapleItemInformationProvider.getInstance().isKarmaEnabled(item.getItemId()) || MapleItemInformationProvider.getInstance().isPKarmaEnabled(item.getItemId())) {
            ...
            case 5_061_000: { // Sealing Lock 7 days
                final MapleInventoryType type = MapleInventoryType.getByType((byte) slea.readInt());
                final IItem item = c.getPlayer().getInventory(type).getItem((byte) slea.readInt());
                // another int here, lock = 5A E5 F2 0A, 7 day = D2 30 F3 0A
                if (item != null && item.getExpiration() == -1) {
                    ...
                    flag |= ItemFlag.LOCK.getValue();
                    item.setFlag(flag);
                    item.setExpiration(System.currentTimeMillis() + (7 * 24 * 60 * 60 * 1_000));
            ...
            case 5_061_001: { // Sealing Lock 30 days
                final MapleInventoryType type = MapleInventoryType.getByType((byte) slea.readInt());
                final IItem item = c.getPlayer().getInventory(type).getItem((byte) slea.readInt());
                // another int here, lock = 5A E5 F2 0A, 7 day = D2 30 F3 0A
                if (item != null && item.getExpiration() == -1) {
                    ...
                    item.setExpiration(System.currentTimeMillis() + (30 * 24 * 60 * 60 * 1_000));
            ...
            case 5_061_002: { // Sealing Lock 90 days
                final MapleInventoryType type = MapleInventoryType.getByType((byte) slea.readInt());
                final IItem item = c.getPlayer().getInventory(type).getItem((byte) slea.readInt());
                // another int here, lock = 5A E5 F2 0A, 7 day = D2 30 F3 0A
                if (item != null && item.getExpiration() == -1) {
                    ...
                    item.setExpiration(System.currentTimeMillis() + (90 * 24 * 60 * 60 * 1_000));
            ...
            case 5_060_003: {//peanut
                IItem item = c.getPlayer().getInventory(MapleInventoryType.ETC).findById(4_170_023);
            ...
            case 5_070_000: { // Megaphone
                ...
                if (!c.getChannelServer().getMegaphoneMuteState()) {
                    ...
                    if (c.getPlayer().isPlayer()) {
                        ...
                        System.out.println("[玩家广播頻道 " + c.getPlayer().getName() + "] : " + message);
                    } else if (c.getPlayer().isGM()) {
                        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.serverNotice(2, sb.toString()));
                        System.out.println("[ＧＭ广播頻道 " + c.getPlayer().getName() + "] : " + message);
                    }
                    used = true;
                } else {
            ...
            case 5_077_000: { // 3 line Megaphone
                if (c.getPlayer().getLevel() < 10) {
                    c.getPlayer().dropMessage(5, "必須等級10級以上才可以使用.");
                    break;
                }
            ...
            case 5_073_000: { // Heart Megaphone
                if (c.getPlayer().getLevel() < 10) {
                    c.getPlayer().dropMessage(5, "必須等級10級以上才可以使用.");
                    break;
                }
            ...
            case 5_074_000: { // Skull Megaphone
                if (c.getPlayer().getLevel() < 10) {
                    c.getPlayer().dropMessage(5, "必須等級10級以上才可以使用.");
                    break;
                }
            ...
            case 5_072_000: { // Super Megaphone
                if (c.getPlayer().getLevel() < 10) {
            ...
            case 5_075_000: // MapleTV Messenger
            case 5_075_001: // MapleTV Star Messenger
            case 5_075_002: { // MapleTV Heart Messenger
            ...
            case 5_090_100: // Wedding Invitation Card
            case 5_090_000: { // Note
            ...
            case 5_100_000: { // Congratulatory Song
                c.getPlayer().getMap().broadcastMessage(MTSCSPacket.playCashSong(5_100_000, c.getPlayer().getName()));
                used = true;
                break;
            }
            case 5_190_001:
            ...
            case 5_170_000: { // Pet name change
/*
                 * final int uniqueid = (int) slea.readLong();
                 */
                MaplePet pet = c.getPlayer().getPet(0);
...
//                }
            ...
            case 5_240_028: { // Pet food
                ...
                if (pet.getCloseness() < 30_000) {
                    if (pet.getCloseness() + 100 > 30_000) {
                        pet.setCloseness(30_000);
                    } else {
            ...
            case 5_281_001: //idk, but probably
            case 5_280_001: // Gas Skill
            case 5_281_000: { // Passed gas
                ...
                c.getPlayer().getMap().spawnMist(mist, 10_000, true);
            ...
            case 5_370_000: { // Chalkboard
                if (c.getPlayer().getMapId() / 1_000_000 == 109) {
            ...
            case 5_370_001: { // BlackBoard
                if (c.getPlayer().getMapId() / 1_000_000 == 910) {
                    c.getPlayer().setChalkboard(slea.readMapleAsciiString());
                }
                break;
            }
            case 5_390_000: // Diablo Messenger
            case 5_390_001: // Cloud 9 Messenger
            case 5_390_002: // Loveholic Messenger
            case 5_390_003: // New Year Megassenger 1
            case 5_390_004: // New Year Megassenger 2
            case 5_390_005: // Cute Tiger Messenger
            case 5_390_006: { // Tiger Roar's Messenger
                if (c.getPlayer().getLevel() < 10) {
            ...
            case 5_450_000: { // Mu Mu the Travelling Merchant
            ...
            case 5_500_002: {
                ...
                MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                int days = 20;
                if (item != null && !GameConstants.isAccessory(item.getItemId()) && item.getExpiration() > -1 && !ii.isCash(item.getItemId()) && System.currentTimeMillis() + (100 * 24 * 60 * 60 * 1_000L) > item.getExpiration() + (days * 24 * 60 * 60 * 1_000L)) {
                    boolean change = true;
                    for (String z : GameConstants.RESERVED) {
                        if (c.getPlayer().getName().contains(z) || item.getOwner().contains(z)) {
                            change = false;
                    ...
                    if (change) {
                        item.setExpiration(item.getExpiration() + (days * 24 * 60 * 60 * 1_000));
                        c.getPlayer().forceReAddItem(item, MapleInventoryType.EQUIPPED);
                        used = true;
                    } else {
    ...
    public static final void Pickup_Pet(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            final double Distance = Client_Reportedpos.distanceSq(mapitem.getPosition());
            if (Distance > 10_000 && (mapitem.getMeso() > 0 || mapitem.getItemId() != 4_001_025)) {
            ...
            } else if (MapleItemInformationProvider.getInstance().isPickupBlocked(mapitem.getItemId()) || mapitem.getItemId() / 10_000 == 291) {
                c.getSession().write(MaplePacketCreator.enableActions());
            } else if (useItem(c, mapitem.getItemId())) {
                removeItem_Pet(chr, mapitem, petz);
            } else if (MapleInventoryManipulator.checkSpace(c, mapitem.getItemId(), mapitem.getItem().getQuantity(), mapitem.getItem().getOwner())) {
                if (mapitem.getItem().getQuantity() >= 50 && mapitem.getItemId() == 2_340_000) {
                    c.setMonitored(true); //hack check
                }
    ...
    private static final boolean getIncubatedItems(MapleClient c) {
        ...
        final int[] ids = {2_430_091, 2_430_092, 2_430_093, 2_430_101, 2_430_102, //mounts
            2_340_000, //rares
            1_152_000, 1_152_001, 1_152_004, 1_152_005, 1_152_006, 1_152_007, 1_152_008, //toenail only comes when db is out.
            1_000_040, 1_102_246, 1_082_276, 1_050_169, 1_051_210, 1_072_447, 1_442_106, //blizzard
            3_010_019, //chairs
            1_001_060, 1_002_391, 1_102_004, 1_050_039, 1_102_040, 1_102_041, 1_102_042, 1_102_043, //equips
            1_082_145, 1_082_146, 1_082_147, 1_082_148, 1_082_149, 1_082_150, //wg
            2_043_704, 2_040_904, 2_040_409, 2_040_307, 2_041_030, 2_040_015, 2_040_109, 2_041_035, 2_041_036, 2_040_009, 2_040_511, 2_040_408, 2_043_804, 2_044_105, 2_044_903, 2_044_804, 2_043_009, 2_043_305, 2_040_610, 2_040_716, 2_041_037, 2_043_005, 2_041_032, 2_040_305, //scrolls
            2_040_211, 2_040_212, 1_022_097, //dragon glasses
            2_049_000, 2_049_001, 2_049_002, 2_049_003, //clean slate
            1_012_058, 1_012_059, 1_012_060, 1_012_061, //pinocchio nose msea only.
            1_332_100, 1_382_058, 1_402_073, 1_432_066, 1_442_090, 1_452_058, 1_462_076, 1_472_069, 1_482_051, 1_492_024, 1_342_009, //durability weapons level 105
            2_049_400, 2_049_401, 2_049_301};
        //out of 1000
        ...
        while (chances[z] < Randomizer.nextInt(1_000)) {
            z = Randomizer.nextInt(ids.length);
        }
        int z_2 = Randomizer.nextInt(ids.length);
        while (z_2 == z || chances[z_2] < Randomizer.nextInt(1_000)) {
    ...
    public static final void OwlMinerva(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        ...
        final int itemid = slea.readInt();
        final IItem toUse = c.getPlayer().getInventory(MapleInventoryType.USE).getItem(slot);
        if (toUse != null && toUse.getQuantity() > 0 && toUse.getItemId() == itemid && itemid == 2_310_000) {
            final int itemSearch = slea.readInt();
            final List<HiredMerchant> hms = c.getChannelServer().searchMerchant(itemSearch);
            if (hms.size() > 0) {
                c.getSession().write(MaplePacketCreator.getOwlSearched(itemSearch, hms));
    ...
    public static final void Owl(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer().haveItem(5_230_000, 1, true, false) || c.getPlayer().haveItem(2_310_000, 1, true, false)) {
            if (c.getPlayer().getMapId() >= 910_000_000 && c.getPlayer().getMapId() <= 910_000_022) {
    ...
    public static final void UseSkillBook(final SeekableLittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        for (;;) {
            ...
            if (Math.floor(CurrentLoopedSkillId / 10_000) == chr.getJob()) {
    ...
    public static final void OwlWarp(final SeekableLittleEndianAccessor slea, final MapleClient c) {
        c.getSession().write(MaplePacketCreator.enableActions());
        if (c.getPlayer().getMapId() >= 910_000_000 && c.getPlayer().getMapId() <= 910_000_022 && c.getPlayer().getPlayerShop() == null) {
            final int id = slea.readInt();
            final int map = slea.readInt();
            if (map >= 910_000_001 && map <= 910_000_022) {
