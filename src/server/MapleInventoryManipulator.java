...
public class MapleInventoryManipulator {
    ...
    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, byte Flag) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isRechargable(itemId)) {
                ...
                while (quantity > 0) {
                    ...
                    if (newQ != 0) {
                        ...
                        if (period > 0) {
                            nItem.setExpiration(System.currentTimeMillis() + (period * 24 * 60 * 60 * 1_000));
                        }
                        if (pet != null) {
                            nItem.setPet(pet);
            ...
            if (period > 0) {
                nEquip.setExpiration(System.currentTimeMillis() + (period * 24 * 60 * 60 * 1_000));
            }
    ...
    public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
         ...
         * slotMax) {
         * c.getSession().write(MaplePacketCreator.moveAndMergeWithRestInventoryItem(type,
         * src, dst, (short) ((olddstQ + oldsrcQ) - slotMax), slotMax)); } else
         * {
         * c.getSession().write(MaplePacketCreator.moveAndMergeInventoryItem(type,
         * src, dst, ((Item)
         * c.getPlayer().getInventory(type).getItem(dst)).getQuantity())); } }
         * else {
         * c.getSession().write(MaplePacketCreator.moveInventoryItem(type, src,
         * dst)); }
         */
    }
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (dst < -999 && !GameConstants.isEvanDragonItem(source.getItemId()) && !GameConstants.is豆豆装备(source.getItemId())) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        } else if (dst >= -999 && dst < -99 && stats.get("cash") == 0 && !GameConstants.is豆豆装备(source.getItemId())) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
        ...
        if (GameConstants.isEvanDragonItem(source.getItemId()) && (chr.getJob() < 2_200 || chr.getJob() > 2_218)) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        if (c.getPlayer().getCurrenttime() - c.getPlayer().getLasttime() < 300) {
