            ...
            try {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        MapleNPC npc = MapleLifeFactory.getNPC(spawnsid);
                        if (npc != null && !npc.getName().equals("MISSINGNO")) {      //独家修复NPC消失问题
        ...
        try {
            ...
            try (java.sql.PreparedStatement ps = con.prepareStatement("SELECT * FROM `wz_customlife`"); ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    final AbstractLoadedMapleLife myLife = loadLife(rs.getInt("dataid"), rs.getInt("f"), rs.getByte("hide") > 0, rs.getInt("fh"), rs.getInt("cy"), rs.getInt("rx0"), rs.getInt("rx1"), rs.getInt("x"), rs.getInt("y"), rs.getString("type"));
                    if (myLife == null) {
                        continue;
                    }
